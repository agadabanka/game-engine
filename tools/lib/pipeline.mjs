...
export const PIPELINE = [
  ...
  { key: 'deconstruct', phase: 'design', title: 'Deconstruct — the mechanics SPEC (the missing input)',
    sub: ['route concept → genre', 'write mechanics.json (verbs+tuning · tech/chains · contraptions · room-grammar · curve · failure)', 'a human edits the numbers / adds game-specific tech'],
    accept: 'Run `node tools/deconstruct.mjs <dir>`: `mechanics.json` exists and validates (the six parts). When you say "clone <game>", THIS captures the verb-set + the chains that ARE the depth, so the solver + level builder read it instead of re-inventing it.' },
  ...
  { key: 'art', phase: 'make', title: 'Art — backdrops + parallax + title keyart',
    sub: ['a backdrop per world', 'multi-layer PARALLAX layers per world (depth)', 'title keyart', 'sprite sheets + tiles + props', 'gameplay-clean, no text in images'],
    accept: 'One command: `node tools/full-art.mjs <dir>` (backdrops + multi-layer parallax layers (`Studio.Parallax`) + title keyart + hero/enemy sheets + tiles + props from `GAME_META.art`, chroma-keyed + packed, `gencache`d). Every world has depth-separated parallax; bottom third gameplay-clean; NO text.' },
  ...
  { key: 'gate', phase: 'ship', title: 'Gate — eval + PARITY GREEN',
    sub: ['menu / human-path smoke (0 errors)', 'autopilot wins 0-death, both renderers', 'felt-gate (FUN≥70 / MIRTH≥65)', 'parity gate (tools/parity.mjs)', 'preflight ship-doctor (tools/preflight.mjs)'],
    accept: 'Menu/human-path smoke (0 page errors) + autopilot WINS every level 0-death, deterministic, non-black on BOTH renderers, the felt-gate passes, the parity gate (`tools/parity.mjs`) passes (menu + mobile + touch + real art + ≥3 mechanics + ≥2 enemy kinds + a story), AND the preflight ship-doctor (`tools/preflight.mjs`) passes — the regression battery for every bug this engine has shipped: no Phaser audio loading (iOS), width-gated portrait rotate, props with real alpha, edge-to-edge tiles, title keyart, on-theme platform mats, and a committed UI click test.' },
