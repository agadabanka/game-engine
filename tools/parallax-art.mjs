...
const LAYERS = (world) => [
  { tag: 'sky', opaque: true,
    prompt: `A wide panoramic dreamy SKY for a bright whimsical platformer world called "${world}". Soft pastel gradient, a big soft glowing sun and a few faint distant clouds high up. NO ground, NO hills, NO mountains, NO characters, NO text. Painterly, gentle, fills the whole frame, seamlessly tileable left-to-right.` },
  { tag: 'mountains',
    prompt: `A wide panoramic row of DISTANT soft pastel MOUNTAINS and big translucent RAINBOW ARCS for a whimsical world called "${world}", sitting only along the BOTTOM HALF of the image, hazy and far away. ${GREEN} Seamlessly tileable left-to-right.` },
  { tag: 'hills',
    prompt: `A wide panoramic row of rolling FOREGROUND CANDY HILLS and bright grass for a whimsical world called "${world}", filling only the BOTTOM THIRD of the image, saturated and close-up with chunky soft shapes. ${GREEN} Seamlessly tileable left-to-right.` },
];
...
for (const w of pick) {
  const recipes = LAYERS(w.name), files = [];
  console.log(`\n${w.name}:`);
  for (let li = 0; li < recipes.length; li++) {
    const rec = recipes[li], file = `${slug(w.name)}-L${li}.png`, dst = path.join(outDir, file);
    if (fs.existsSync(dst) && !FORCE) { files.push({ file, scroll: [0.08, 0.25, 0.5][li] }); console.log(`  – L${li} ${rec.tag} (exists)`); continue; }
    try {
      const { base64 } = await generateImage(rec.prompt, { aspectRatio: '16:9' });
