...
- **What's still scoped:** the solver covers run/jump/dash/wall-jump + spikes; springs and richer contraptions (dream-block/bumper/mover) are the **contraption-registry** follow-up — each contraption needs (a) runtime behavior, (b) a solver model, (c) an authoring builder. Add models as you add gadgets.
